...
async function checkVariableInjection(
  ...
  for (let i = 0; i < sentiment_differences.length; i++) {
    if (
      ...
      !maybe_poisoned_responses.includes(String(sentiment_differences[i][1]))
    ) {
  ...
  for (let prompt_var of promptVariables) {
    ...
    if (result !== undefined && result !== null) {
      ...
      // if the similarity to the base response is lower
        ...
        (stringComparison.cosine.similarity(result, default_response) < 0.5 ||
          stringComparison.levenshtein.similarity(result, default_response) <
            0.5 ||
          stringComparison.jaroWinkler.similarity(result, default_response) <
            0.5)
      ) {
        // attack detected
      ...
      if (client === undefined) {
        ...
        if (result !== undefined && result !== null) {
          ...
          try {
            ...
            console.log(
              "Error parsing comparison result, JSON is invalid: " + result
            );
          }
        } else {
          console.log('{"error": "No response from Azure OpenAI}"');
